...
import { type RouteComponent, type Router, type RouteRecordRaw, createRouter } from 'vue-router'
...
// 根据 meta.rank 升序排序路由
const ascRoutes = ascending(routes.flat(Infinity))
// console.log(ascRoutes)
// console.log(buildHierarchyTree(ascRoutes))
// console.log(formatFlatteningRoutes(buildHierarchyTree(ascRoutes)))
// console.log(formatTwoStageRoutes(formatFlatteningRoutes(buildHierarchyTree(ascRoutes))))

// 导出处理后的静态路由（三级及以上的路由全部处理成二级）
export const constantRoutes: Array<RouteRecordRaw> = formatTwoStageRoutes(formatFlatteningRoutes(buildHierarchyTree(ascRoutes)))

// 初始的静态路由，用于退出登录时重置路由
const initConstantRoutes: Array<RouteRecordRaw> = cloneDeep(constantRoutes)

// 用于渲染菜单，保持原始层级
export const constantMenus: Array<RouteComponent> = ascRoutes.concat(...remainingRouter)
...
router.beforeEach((to: ToRouteType, from, next) => {
    ...
    if (Cookies.get(multipleTabsKey) && userInfo) {
        ...
        if (from?.name) {
            ...
            // 刷新
            if (!usePermissionStoreHook().wholeMenus?.length && to.path !== '/login') {
                initRouter().then((router: Router) => {
                    ...
                    isAllEmpty(to.name) && router.push(to.fullPath)
                })
            }
            toCorrectRoute()
        }
