...
export const addPreventDefault = () => {
    ...
    useEventListener(window.document, 'keydown', ev => ev.key === 'F12' && ev.preventDefault())
    ...
    useEventListener(window.document, 'dragstart', ev => isImgElement(ev?.target) && ev.preventDefault())
}
